//----------------------------------------------------------------------------------------------
/*
 * Creator             : Irfan Indra Kurniawan, ST
 * Website             : https://www.fanselectronics.com
 * Buy Me a Coffee     : https://saweria.co/FansElectronics
 * Ported for Multi-Panel Type (HUB08 & HUB12) on ESP32
 * Support P10, P4.25, P7.62, dan panel custom dengan konfigurasi pin fleksibel
 */

#ifndef FansElectronics_Bitmap_h
#define FansElectronics_Bitmap_h

#include "inttypes.h"
#include "pgmspace.h"
#include <WString.h>
#include <string.h>
#include <stdlib.h>

class FansElectronics_DMD8266;
class String;

//----------------------------------------------------------------------------------------------
// ID: Kelas Induk Grafis Berbasis Manajemen Koordinat Piksel Monokrom
// EN: Base Graphic Class Managing Monochrome Coordinate Framebuffers
class FansElectronics_Bitmap
{
public:
    FansElectronics_Bitmap(int width, int height);
    ~FansElectronics_Bitmap();
    bool isValid() const { return fb != 0; }
    typedef uint8_t Color;
    typedef PGM_VOID_P ProgMem;
    typedef PGM_VOID_P Font;
    static const Color Black = 0;
    static const Color White = 1;
    static const Color NoFill = 2;
    int width() const { return _width; }
    int height() const { return _height; }
    int stride() const { return _stride; }
    int bitsPerPixel() const { return 1; }
    uint8_t *data() { return fb; }
    const uint8_t *data() const { return fb; }

    // ID: Prototipe fungsi manipulasi primitif dan pencetakan font
    // EN: Prototypes of canvas primitive shapes and text-rendering APIs
    void clear(Color color = Black);
    Color pixel(int x, int y) const;
    void setPixel(int x, int y, Color color);
    void drawLine(int x1, int y1, int x2, int y2, Color color = White);
    void drawRect(int x1, int y1, int x2, int y2, Color borderColor = White, Color fillColor = NoFill);
    void drawFilledRect(int x1, int y1, int x2, int y2, Color color = White);
    void drawCircle(int centerX, int centerY, int radius, Color borderColor = White, Color fillColor = NoFill);
    void drawFilledCircle(int centerX, int centerY, int radius, Color color = White);
    void drawBitmap(int x, int y, const FansElectronics_Bitmap &bitmap, Color color = White);
    void drawBitmap(int x, int y, FansElectronics_Bitmap::ProgMem bitmap, Color color = White);
    void drawInvertedBitmap(int x, int y, const FansElectronics_Bitmap &bitmap);
    void drawInvertedBitmap(int x, int y, FansElectronics_Bitmap::ProgMem bitmap);
    Font font() const { return _font; }
    void setFont(Font font) { _font = font; }
    Color textColor() const { return _textColor; }
    void setTextColor(Color color) { _textColor = color; }
    void drawText(int x, int y, const char *str, int len = -1);
    void drawText(int x, int y, const String &str, int start = 0, int len = -1);
    void drawTextCenter(int max_x, int x, int y, const String &str);
    int drawChar(int x, int y, char ch);
    int charWidth(char ch) const;
    int textWidth(const char *str, int len = -1) const;
    int textWidth(const String &str, int start = 0, int len = -1) const;
    int textHeight() const;
    void copy(int x, int y, int width, int height, FansElectronics_Bitmap *dest, int destX, int destY);
    void fill(int x, int y, int width, int height, Color color);
    void fill(int x, int y, int width, int height, FansElectronics_Bitmap::ProgMem pattern, Color color = White);
    void scroll(int dx, int dy, Color fillColor = Black);
    void scroll(int x, int y, int width, int height, int dx, int dy, Color fillColor = Black);
    void invert(int x, int y, int width, int height);

private:
    friend class FansElectronics_DMD8266;
    FansElectronics_Bitmap(const FansElectronics_Bitmap &) {}
    FansElectronics_Bitmap &operator=(const FansElectronics_Bitmap &) { return *this; }
    int _width;
    int _height;
    int _stride;
    uint8_t *fb;
    Font _font;
    Color _textColor;
    void blit(int x1, int y1, int x2, int y2, int x3, int y3);
    void drawCirclePoints(int centerX, int centerY, int radius, int x, int y, Color borderColor, Color fillColor);
};

// ID: Implementasi fungsi segaris (inline) untuk optimasi kecepatan kompilator
// EN: Inline method implementations guaranteeing fast compiler execution bounds
inline void FansElectronics_Bitmap::drawFilledRect(int x1, int y1, int x2, int y2, Color color)
{
    drawRect(x1, y1, x2, y2, color, color);
}
//----------------------------------------------------------------------------------------------
inline void FansElectronics_Bitmap::drawFilledCircle(int centerX, int centerY, int radius, Color color)
{
    drawCircle(centerX, centerY, radius, color, color);
}
//----------------------------------------------------------------------------------------------
inline void FansElectronics_Bitmap::drawInvertedBitmap(int x, int y, const FansElectronics_Bitmap &bitmap)
{
    drawBitmap(x, y, bitmap, Black);
}
//----------------------------------------------------------------------------------------------
inline void FansElectronics_Bitmap::drawInvertedBitmap(int x, int y, FansElectronics_Bitmap::ProgMem bitmap)
{
    drawBitmap(x, y, bitmap, Black);
}
//----------------------------------------------------------------------------------------------
inline void FansElectronics_Bitmap::scroll(int dx, int dy, Color fillColor)
{
    scroll(0, 0, _width, _height, dx, dy, fillColor);
}
//----------------------------------------------------------------------------------------------
#endif